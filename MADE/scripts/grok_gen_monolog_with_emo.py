...
OUTPUT_DIR = "dataset/MADE/grok_monologues_with_emo"
...
def generate_monologues():
    """Generate monologues using Grok API"""
    try:
        ...
        for mono in monologues:
            for s in mono.get("sentences", []):
                if "emotion" not in s or "text" not in s:
                    print("Missing emotion/text key, skipping batch.")
                    return []
        return monologues
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print("Starting Grok generation with local + global duplicate filtering...\n")
    ...
    while True:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if monologues:
                break
            print(f"Empty or invalid response, retrying ({attempt+1}/{MAX_RETRIES})")
